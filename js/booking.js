(function () {
	...
	(function bookingWizard() {
		...
		function isAppointmentOnlyDay(dateStr) {
			if (!dateStr) return false;
			var day = new Date(dateStr + "T00:00:00").getDay();
			return day === 5 || day === 6;
		}
		...
		function validForStep(step) {
			...
			if (step === 4) return !!state.washType && captchaCompleted();
			return true;
		}
		...
		function renderVehicleCards() {
			...
			vehicleMeta.forEach(function (v) {
				...
				var priceLabel = (v.id === "larger" || isAppointmentOnlyDay(state.date)) ? "Call for Price" : ("From $" + money(priceTable[v.id].outside) + " BZD");
		...
		function isCallForPrice() {
			return state.vehicle === "larger" || state.extraDirty || isAppointmentOnlyDay(state.date);
		}
		...
		var retryBtn = document.getElementById("bk-retry");
		...
		function renderConfirmation() {
			confirmSuccess.hidden = !state.submissionOk;
			confirmError.hidden = !!state.submissionOk;
			if (!state.submissionOk) return;
		...
		function bookingSummary() {
			var lines = [
				...
				"Wash Type: " + washLabel(),
				"Requested Date/Time: " + formatSchedule(state.date, state.time),
				"Extra Dirty: " + (state.extraDirty ? "Yes" : "No"),
				"Tire Shine: " + (state.tireShine ? "Yes (+$10 BZD)" : "No")
			];
			lines.push(isCallForPrice() ? "Pricing: Call to confirm" : "Total: $" + money(total()) + " BZD");
			return lines.join("\n");
		}
		function submitBooking() {
			var captchaEl = captchaResponseEl();
			var payload = {
				...
				"h-captcha-response": captchaEl ? captchaEl.value : ""
			};
			var timeout = new Promise(function (resolve) {
				setTimeout(function () { resolve(false); }, 12000);
			});
			var request = fetch("https://api.web3forms.com/submit", {
				method: "POST",
		...
		function clearErrors() {
			["name", "phone", "email", "date", "time", "vehicle", "wash", "captcha"].forEach(function (k) {
		...
		function showErrors() {
			...
			toggleErr("captcha", !!state.washType && !captchaCompleted(), null);
			...
			if (state.date && !isFutureOrToday(state.date)) {
				...
				setErrText("date", "We’re closed Sundays, pick another day.");
			} else {
		...
		continueBtn.addEventListener("click", function () {
			...
			if (state.step === 4) {
				beginTransition("Confirming your booking…");
				submitBooking().then(function (ok) {
					endTransition();
					state.submissionOk = ok;
					state.step = 5;
		...
		resetBtn.addEventListener("click", function () {
			if (stepTimer) clearTimeout(stepTimer);
			state = { step: 1, showErrors: false, submissionOk: false, name: "", phone: "", phoneCode: "+501", email: "", date: "", time: "", vehicle: "", washType: "", extraDirty: false, tireShine: false };
